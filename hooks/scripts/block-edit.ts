    ...
    JSON.stringify({
      hookSpecificOutput: {
        ...
        permissionDecisionReason:
          "Built-in Edit/Write are disabled by the hashline plugin. Use the hashline edit tool (mcp__plugin_claude-hashline_hashline__edit): read the file to get its [PATH#TAG], then send line-anchored ops (PUT N.=M:, PUT >N:, CUT N.=M). Create a file with a tagless [path] header + PUT <1: body. To opt this repo out: add a .hashline-off file at the repo root, or set HASHLINE_DISABLED=1, or create ~/.hashline-off.",
      },
    }),
  );
}
